...
export class AnswerListComponent implements OnInit {
  ...
  editAnswer(id) {
    this.rest.editAnswer(this.editanswer.value, id).subscribe((data: { message: any }) => {
      if (data.message == 'answer-updated') {
        $(".close").click();
          this.getAnswer(this.question_id);
          this.toastr.success('Answer Updated', 'Success!');
          this.router.navigateByUrl('/answerlist/' +this.question_id);
      } else {
  ...
  deleteAnswerInfo(id){
    this.rest.deleteAnswer(id).subscribe((data: { message: any }) => {
      if (data.message == 'answer-deleted') {
        ...
        $(".close").click();
        this.toastr.error('Answer not Deleted', 'Oops!');
        this.router.navigateByUrl('/answerlist/' +this.question_id);
      } 
    });
  }
