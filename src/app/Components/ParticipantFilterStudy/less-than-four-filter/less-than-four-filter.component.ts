...
@Component({
  selector: 'app-less-than-four-filter',
  templateUrl: './less-than-four-filter.component.html',
  styleUrls: ['./less-than-four-filter.component.sass']
})
export class LessThanFourFilterComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    // this.getParticipantStudyLessThanFourList();
  }

    
  // getParticipantStudyLessThanFourList() {
  //   this.rest.getParticipantRatingList().subscribe((data: { message:any, Data:any }) => {
  //     if(data.message == 'unauthorised-user'){
  //       this.router.navigateByUrl('/');
  //     }else{
  //       this.participantfilterlist = data.Data.between_two_four;
  //     }
  //   });
  // }
  ...
  whiteListNewUser(user_id){
    ...
    this.rest.whiteListNewUser(user_id,this.study_id).subscribe((data: { message: any }) => {
      if (data.message == 'user-white-listed') {
        ...
        this.router.navigateByUrl('/studylessthanfourfilter/' +this.study_id);
