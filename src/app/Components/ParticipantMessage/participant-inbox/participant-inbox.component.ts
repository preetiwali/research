...
@Component({
  selector: 'app-participant-inbox',
  templateUrl: './participant-inbox.component.html',
  styleUrls: ['./participant-inbox.component.sass']
})
export class ParticipantInboxComponent implements OnInit {
  ...
  deleteMail(id) {
    this.rest.deleteMail(id).subscribe((data: { message: any }) => {
      if (data.message == 'message-deleted') {
        $(".close").click();
        this.router.navigateByUrl('/participantinbox');
        this.inboxMessage();
      } else {
        $(".close").click();
        this.router.navigateByUrl('/participantinbox');
