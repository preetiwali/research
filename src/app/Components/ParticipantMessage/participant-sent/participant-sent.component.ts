...
@Component({
  selector: 'app-participant-sent',
  templateUrl: './participant-sent.component.html',
  styleUrls: ['./participant-sent.component.sass']
})
export class ParticipantSEntComponent implements OnInit {
  ...
  deleteMail(id) {
    this.rest.deleteMail(id).subscribe((data: { message: any }) => {
      if (data.message == 'message-deleted') {
        $(".close").click();
        this.router.navigateByUrl('/participantsent');
        this.sentMessage();
      } else {
        $(".close").click();
        this.router.navigateByUrl('/participantsent');
