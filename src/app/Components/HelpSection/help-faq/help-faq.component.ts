...
@Component({
  ...
  styleUrls: ['./help-faq.component.sass']
})
export class HelpFAQComponent implements OnInit {
  ...
  getHelpFAQ() {
    this.rest.getParticipantHelpFAQ().subscribe((data: { message:any, Data: { question_categories:any } }) => {
  ...
  orderParticipantFAQ(id) {
    this.rest.editHelpQuestion(this.orderfaq.value, id).subscribe((data: { message: any, id: any }) => {
      if (data.message == 'faq-updated') {
          ...
          this.router.navigateByUrl('/helpFAQ');
      } else {
        this.getHelpFAQ();
  ...
  addHelpQuestions() {
    this.addhelpquestion.value.user_type = 'Participant';
  ...
  deleteParticipantFAQImage(id){
    this.rest.deleteParticipantFAQImage(id).subscribe((data: { message: any }) => {
    ...
    $('.user_id_proof_img').removeClass('display_none');
      // this.updateProfileImageForm.value
        if(this.selectedFile != null)
        {
          ...
          this.rest.updateFaqImage(fd,id).subscribe((data: { message: any }) => {
